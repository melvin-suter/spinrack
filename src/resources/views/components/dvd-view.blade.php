<a href="{{ isset($new) && $new ? '/check/'.$dvd->media_type.'/'.$dvd->tmdbid : '/dvd/'.$dvd->id }}" class="dvd">
    @if(isset($new) && $new)
    ...
    @endif
    <div class="front">
        <strong style="font-size: 0.9rem">{{$dvd->title}}</strong>
        <div>
            @if($dvd->season)
                <strong style="font-size: 0.9rem">{{$dvd->season_name}}</strong>
            @endif
            @if(!isset($new) || !$new)
